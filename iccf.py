...
# -*- coding: utf-8 -*-
"""
Stockfish FEN analyser + Max-Strength Best-Move + Lichess Explorer Assist

What’s in:
- Always MAX engine strength (no Elo limiting, no styles).
- Explorer assist kept (weight, Min games, Top moves) + Cloud Eval toggle.
- Blue arrow = current picked move (engine best + optional Explorer bias).
- Orange arrow = temporary “Best move” preview button (raw engine line).
- Status bar shows BookEV, real G: (never hidden), and “(filtered)” when under Min games.
...
BOARD_SIZE = 360
SQUARE = BOARD_SIZE // 8
K_LOG = 0.004  # win% curve for headline
...
_http.headers.update({"User-Agent": "ICCF-Assistant/MaxStrength/1.0 (+contact@example.com)"})
HTTP_TIMEOUT = 7.0
...
def lichess_explorer(fen: str, top_moves: int = 50) -> Optional[dict]:
    """Explorer request (no speeds filter)."""
...
# ---------- Explorer EV / counts (always show real counts) ----------
def explorer_bonus_for_move(move_uci: str, ex_data: dict, min_sample: int):
    ...
    tot = w + d + b
    games = int(found.get("gameCount") or tot)  # infer when gameCount missing
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Stockfish (Max) + Lichess Explorer Assist")
        ...
        try:
            ...
            # Run full strength; set threads/hash as desired
    ...
    def redraw(self, board: Optional[chess.Board] = None, highlight: Optional[chess.Move] = None):
        ...
        arrows = []
        # Blue arrow for current picked move (if it's our turn so it makes sense)
        if self.pick_move and self.board.turn == (chess.WHITE if self.my_color_is_white() else chess.BLACK):
            arrows.append(chess.svg.Arrow(self.pick_move.from_square, self.pick_move.to_square, color="#3b82f6"))
        if highlight:
    ...
    def _finish_eval(self, gen: int, fen: str, info_list: list,
        ...
        # Choose pick (engine cp + Explorer bias)
    ...
    # ---------- Pick move: engine best + Explorer bonus ----------
